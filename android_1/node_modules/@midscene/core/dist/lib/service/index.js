...
class Service {
    async locate(query, opt, modelConfig) {
        ...
        const { parseResult, rect, rawResponse, usage } = await (0, index_js_namespaceObject.AiLocateElement)({
    ...
    async describe(target, modelConfig, opt) {
        ...
        if (opt?.deepThink) {
            const searchArea = (0, external_common_js_namespaceObject.expandSearchArea)(targetRect, context.size, vlMode);
            debug('describe: set searchArea', searchArea);
            const croppedResult = await (0, img_namespaceObject.cropByRect)(imagePayload, searchArea, 'qwen2.5-vl' === vlMode);
        ...
        const res = await callAIFn(msgs, external_common_js_namespaceObject.AIActionType.DESCRIBE_ELEMENT, modelConfig);
        const { content } = res;
